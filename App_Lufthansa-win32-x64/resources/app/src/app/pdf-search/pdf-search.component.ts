...
export class PdfSearchComponent {
  ...
  downloadFileName: string = 'results.xlsx';
  ...
  async onFileSelected(event: any) {
    ...
    if (selectedFile) {
      ...
      if (pdfData) {
        this.pdfSrc = pdfData;
        // await this.searchAndSaveToExcel(pdfData);
      } else {
        console.log('Неуспешно зареждане на PDF файл.');
      }
    }
  }
  async onFileExcelUpload(event: any) {
    ...
    if (selectedFile) {
      ...
      console.log('Неуспешно зареждане на Excel файл.');
    }
  }
  

  // async searchAndSaveToExcel(pdfData: string) {
  ...
  //   const matches = textContent.map(row => row[0]).join('\n').match(regex);
  ...
  //   } else {
  //     console.log('Няма съвпадения с регулярния израз.');
  //   }
  // }
